...
EXTRA_ROS_FILES := $(shell find $(EXTRA_ROS_PACKAGES) -type f 2>/dev/null)
UROS_INSTALL_STAMP := $(UROS_DIR)/install/.laksa_interfaces_stamp
...
$(EXTENSIONS_DIR)/micro_ros_dev/install:
	...
	colcon build --executor sequential \
		--cmake-args -DBUILD_TESTING=OFF -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++;
...
$(UROS_INSTALL_STAMP): $(EXTENSIONS_DIR)/esp32_toolchain.cmake $(EXTENSIONS_DIR)/micro_ros_dev/install $(EXTENSIONS_DIR)/micro_ros_src/src $(EXTRA_ROS_FILES) libmicroros.mk
	mkdir -p $(UROS_DIR)/src/extra_packages; \
	cp -rfL $(EXTRA_ROS_PACKAGES)/. $(UROS_DIR)/src/extra_packages/ || :; \
	cd $(UROS_DIR); \
	...
	colcon build \
		--executor sequential \
		--merge-install \
		--packages-up-to rclc laksa_interfaces sensor_msgs \
		...
		-DCMAKE_PREFIX_PATH=$(UROS_DIR)/install \
		-Dmicrocdr_DIR=$(UROS_DIR)/install/share/microcdr/cmake \
		-DCMAKE_VERBOSE_MAKEFILE=OFF \
		-DIDF_INCLUDES='${IDF_INCLUDES}' \
		-DCMAKE_C_STANDARD=$(C_STANDARD) \
		-DUCLIENT_C_STANDARD=$(C_STANDARD); \
	touch $(UROS_INSTALL_STAMP);

$(EXTENSIONS_DIR)/libmicroros.a: $(UROS_INSTALL_STAMP) libmicroros.mk
