...
projection for 2D navigation and may not be a second dynamic odometry source.
`robot_state_publisher` publishes only static body/sensor geometry. One local
estimator owns `odom -> base_link`; one SLAM/localization component owns
`map -> odom`.
