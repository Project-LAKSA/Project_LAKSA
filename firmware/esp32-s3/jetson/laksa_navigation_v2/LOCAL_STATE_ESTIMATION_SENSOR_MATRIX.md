...
|---|---|---|---|
| X, Y | ZED continuous local pose, composed camera-to-base | raw `odom -> zed_camera_link` is converted through canonical TF before EKF | retain after covariance review |
...
| yaw | ZED continuous local pose, composed camera-to-base | avoids raw ZED IMU correlation | evaluate independent IMU yaw rate only after correlation study |
