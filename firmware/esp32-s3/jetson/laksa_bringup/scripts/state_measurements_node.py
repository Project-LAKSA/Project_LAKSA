...
class StateMeasurements(Node):
    ...
    def _message_stamp(self, message: VehicleState):
        """Return a current, monotonic ROS timestamp for adapted telemetry.

        ESP32 telemetry may carry a non-zero timestamp from a clock that is
        not synchronized with the Jetson.  Propagating that old stamp into
        robot_localization makes otherwise healthy measurements appear stale
        and can suppress the fused odometry output.  Preserve an incoming
        stamp only when it is close to the Jetson clock and advances; otherwise
        timestamp the bridge output at receipt time.
        """
        now = self.get_clock().now()
        now_ns = now.nanoseconds
        incoming_ns = int(message.stamp.sec) * 1_000_000_000 + int(
            message.stamp.nanosec
        )
        if (
            incoming_ns > 0
            and abs(now_ns - incoming_ns) <= 500_000_000
            and incoming_ns > self._last_output_stamp_ns
        ):
            self._last_output_stamp_ns = incoming_ns
            return message.stamp
        self._last_output_stamp_ns = now_ns
        return now.to_msg()
